...
def for_760(empid,bamboo_input,emp_id,bamboo_name_id,bamboo_ID,date_for_bamboo,converted_input,Approvers_List1_array):
    ...
    try:
        ...
        if (prev_employee != None and len(prev_employee)> 0 ):
            ...
            if (converted_input.get("TPROPRSTLC")!= "Closed") and ("9999" in employee_termination_date):
                ...
                Approvers_List1_array.append(Approvers_List_for_exp)
...
def for_710(empid2,bamboo_input,emp_id,bamboo_name_id,bamboo_ID,date_for_bamboo,converted_input,Approvers_List1_array):
    Bamboo_ID=None
    Employee_ID=None
    Employee_Text=None
    employee_termination_date=None
    ...
    # -------------------------
        ...
        if (converted_input.get("TPROPRSTLC")!="Closed") and ("9999" in employee_termination_date):
            ...
            Approvers_List1_array.append(Approvers_List2_for_exp)
...
def main(input):
    ...
    try:
        from datetime import datetime
        last_run_date_object=datetime.strptime(lastRunDate, "%Y-%m-%dT%H:%M:%S")
        from datetime import datetime
        current_date_time = datetime.now()
    except Exception as e:
        return {"error_list":f"{e} while converting last run date and today date"}
            ...
            try:
                ...
                if last_pl_date != None and  last_pl_date !="":
                    if "." in last_pl_date:
                        ...
                        failed_reason_list.append( f"${e} could not find date parsing type while creating last_pl_date")
            ...
            try:
                ...
                if last_pr_date != None and  last_pr_date !="":
                    if "." in last_pr_date:
                        ...
                        failed_reason_list.append( f"${e}could not find date parsing type while creating last_pr_date")
                        error_list.append(sap_items)
                        continue
                else:
                    last_pr_date=None
            ...
            try:
                ...
                if sap_items in new_project_array:
                    ...
                    if proj_lead!= None and len(proj_lead) >0 :
                        ...
                        result_710=for_710(proj_lead,bamboo_input,emp_id,bamboo_name_id,bamboo_ID,date_for_bamboo,converted_input,Approvers_List1_array)   # proj_lead=PL
                        ...
                        called_count_710=called_count_710+1
                        #calling 710 method to add
                        result_710=for_710(PRF,bamboo_input,emp_id,bamboo_name_id,bamboo_ID,date_for_bamboo,converted_input,Approvers_List1_array)
                        if result_710.get("error")!=None:
                            failed_reason_list.append(result_710.get("error"))
                ...
                if (proj_lead!= None and len(proj_lead) >0) and (PRF!= None and len(PRF) >0) :
                    if (last_pr_date!=None and last_pl_date!=None):
                        if (last_run_date_object<last_pr_date):
                            if (last_pr_date==last_pl_date):
                                ...
                                result=for_760(prev_lead,bamboo_input,emp_id,bamboo_name_id,bamboo_ID,date_for_bamboo,converted_input,Approvers_List1_array)
                                ...
                                called_count_710=called_count_710+1
                                #calling 710 method to add
                                result_710=for_710(proj_lead,bamboo_input,emp_id,bamboo_name_id,bamboo_ID,date_for_bamboo,converted_input,Approvers_List1_array)
                                if result_710.get("error")!=None:
                                    failed_reason_list.append(result_710.get("error"))
                                    error_list.append(sap_items)
                                if result_710.get("Approvers_List")!=None:
                ...
                if (proj_lead!= None and len(proj_lead) >0) and (PRF!= None and len(PRF) >0):
                    if (last_pl_date!=None):
                        if (last_run_date_object<last_pl_date):
                ...
                if (PRF!= None and len(PRF) >0) and (proj_lead== None):
                    ...
                    if (last_pl_date!=None) and  (last_run_date_object<last_pl_date):
